...
bool Trajectory::computeMinMaxMagnitude(int derivative, const std::vector<int>& dimensions,
  ...
  for (size_t segment_idx = 0; segment_idx < segments_.size(); segment_idx++) {
    ...
    if (!segments_[segment_idx].selectMinMaxMagnitudeFromCandidates(
            0.0, segments_[segment_idx].getTime(), derivative, dimensions, candidates,
...
bool Trajectory::closestPoint(const Eigen::Vector3d& pt, Eigen::VectorXd& outClosetPoint,
                              double& outClosestTime, double& outClosestDistance) const {
  if (D_ < 3) {
    return false;
  }

  double endTime = getMaxTime();
  float coarseStep = 0.5;
  double testDistance;
  double minTime = 0;
  double minDistance = 9999999999;
  Eigen::VectorXd testPt;
  Eigen::VectorXd closestPt;

  for (int i = 0; i <= endTime / coarseStep; i++) {
    testPt = evaluate(i * coarseStep).head(3);
    testDistance = (testPt - pt).norm();
    // std::cout << "testing: t=" << (i * coarseStep) << " distance: " << testDistance << std::endl;
    if (testDistance < minDistance) {
      minDistance = testDistance;
      minTime = i * coarseStep;
      closestPt = testPt;
    }
  }

  // binary search for precise estimate
  double step = coarseStep / 2.0;
  int iterations = 0;
  while (step > 0.01) {
    Eigen::VectorXd before, after;
    double beforeTime, afterTime, beforeDistance, afterDistance;

    beforeTime = std::max(minTime - step, 0.0);
    before = evaluate(beforeTime);  // this could return dim4
    beforeDistance = (before.head(3) - pt).norm();
    afterTime = std::min(minTime + step, endTime);
    after = evaluate(afterTime);  //  this could return dim4
    afterDistance = (after.head(3) - pt).norm();
    // std::cout << "bT: " << beforeTime << " before: " << before.transpose() << " bD: " <<
    // beforeDistance << std::endl;
    // std::cout << "aT: " << afterTime << " after: " << after.transpose() << " aD: " <<
    // afterDistance << std::endl;
    if (beforeTime >= 0 && beforeDistance < minDistance) {
      closestPt = before;
      minTime = beforeTime;
      minDistance = beforeDistance;
    } else if (afterTime <= endTime && afterDistance < minDistance) {
      closestPt = after;
      minTime = afterTime;
  ...
  // std::cout << "Closest course time: " << minTime << " distance: " << minDistance << " iter: " <<
  // iterations << " pt: " << closestPt.transpose() << std::endl;
  // write out variables
  outClosetPoint = closestPt;
  outClosestTime = minTime;
