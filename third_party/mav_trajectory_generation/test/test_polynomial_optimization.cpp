...
void checkPath(const Vertex::Vector& vertices,
  ...
  REQUIRE(n_segments == n_vertices - 1);
  ...
  for (size_t i = 0; i < n_segments; ++i) {
    ...
    for (Vertex::Constraints::const_iterator it = v_begin.cBegin();
      ...
      REQUIRE(desired.isApprox(actual, tol));
    }
    for (Vertex::Constraints::const_iterator it = v_end.cBegin();
      ...
      std::stringstream segment_derivative;
      printSegment(segment_derivative, segment, derivative);
      REQUIRE(std::fabs((desired - actual).norm()) <= tol);
    }
    ...
    if (i > 0) {
      ...
      for (size_t derivative = 0; derivative < N / 2; ++derivative) {
        ...
        printSegment(segment_derivative, segment, derivative);
        REQUIRE(
            last_segment_value.isApprox(current_segment_value, tol));
      }
    }
  }
}
...
bool checkCost(double cost_to_check, const std::vector<Segment>& segments,
  ...
  REQUIRE(derivative >= size_t(0));
  bool check_tolerance = relative_tolerance >= 0.0 && relative_tolerance <= 1.0;
  REQUIRE(check_tolerance);
...
TEST_CASE("PathPlanning_TestVertexGeneration1D", "[MavTrajectoryGeneration]") {
  Vertex::Vector vertices;
  const double p_min = -50;
  const double p_max = 50;
  vertices = createRandomVertices1D(max_derivative, 100, p_min, p_max, 0);

  REQUIRE(vertices.front().getNumberOfConstraints() == N / 2);
  REQUIRE(vertices.back().getNumberOfConstraints() == N / 2);

  for (const Vertex& v : vertices) {
    REQUIRE(v.hasConstraint(derivative_order::POSITION) == true);
    Eigen::VectorXd c;
...
TEST_CASE("PathPlanning_TestVertexGeneration3D", "[MavTrajectoryGeneration]") {
  ...
  for (const Vertex& v : vertices) {
    ...
    v.getConstraint(derivative_order::POSITION, &c);
    for (int i = 0; i < 3; ++i) {
      REQUIRE(c[i] <= pos_max[i]);
      REQUIRE(c[i] >= pos_min[i]);
    }
  }
}

TEST_CASE("PathPlanning_A_matrix_inversion", "[MavTrajectoryGeneration]") {
  ...
  for (double t = 1; t <= max_time; t += 1) {
    ...
    Ai_eigen = A.inverse();
    REQUIRE(Ai.isApprox(Ai_eigen, 1.0e-10));
  }
}

TEST_CASE("PathPlanningUnconstrained_1D_10_segments", "[MavTrajectoryGeneration]") {
  Vertex::Vector vertices;
  ...
  REQUIRE(v_max < approximate_v_max * 2.0);
  REQUIRE(a_max < approximate_a_max * 2.0);
  REQUIRE(
      checkCost(opt.computeCost(), segments, derivative_to_optimize, 0.1));
}

TEST_CASE("PathPlanningUnconstrained_1D_50_segments", "[MavTrajectoryGeneration]") {
  Vertex::Vector vertices;
  ...
  REQUIRE(v_max < approximate_v_max * 2.0);
  REQUIRE(a_max < approximate_a_max);
  REQUIRE(
      checkCost(opt.computeCost(), segments, derivative_to_optimize, 0.1));
}

TEST_CASE("PathPlanningUnconstrained_1D_100_segments", "[MavTrajectoryGeneration]") {
  Vertex::Vector vertices;
  ...
  REQUIRE(v_max < approximate_v_max * 5.0);
  REQUIRE(a_max < approximate_a_max * 2.0);
  REQUIRE(
      checkCost(opt.computeCost(), segments, derivative_to_optimize, 0.1));
}

TEST_CASE("PathPlanningUnconstrained_1D_100_segments_high_segment_times",
...
TEST_CASE("PathPlanningUnconstrained_3D_100_segments_high_segment_times",
          "[MavTrajectoryGeneration]") {
  Eigen::VectorXd pos_min(3), pos_max(3);
  pos_min << -10.0, -20.0, -10.0;
  pos_max << 10.0, 20.0, 10.0;
...
TEST_CASE("PathOptimization_1D_segment_extrema_of_magnitude",
          "[MavTrajectoryGeneration]") {
  Vertex::Vector vertices;
  vertices = createRandomVertices1D(max_derivative, 100, -10, 10, 1234);
  const double approximate_v_max = 3.0;
  ...
  for (const Segment& s : segments) {
    ...
    REQUIRE(res.size() == res_template_free.size() - 2);
    for (size_t i = 0; i < res.size(); i++) {
      REQUIRE(res[i] == res_template_free[i + 2]);
    }
  ...
  REQUIRE(std::fabs(v_max_ref - v_max.value) <= 0.01);
  REQUIRE(std::fabs(a_max_ref - a_max.value) <= 0.01);
}

TEST_CASE("PathOptimization3D_segment_extrema_of_magnitude", "[MavTrajectoryGeneration]") {
  Eigen::VectorXd pos_min(3), pos_max(3);
...
TEST_CASE("2_vertices_setup", "[MavTrajectoryGeneration]") {
  const int kDim = 1;
  ...
  REQUIRE(matlab_coeffs.isApprox(coeffs, 0.001));
}

// Test 2 vertices setup
TEST_CASE("2_vertices_rand", "[MavTrajectoryGeneration]") {
...
TEST_CASE("ConstraintPacking", "[MavTrajectoryGeneration]") {
  ...
  for (size_t i = 0; i < kNumSetups; i++) {
    ...
    REQUIRE(fixed_constraints.size() == 3);
    REQUIRE(free_constraints.size() == 3);
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      REQUIRE(d_all_ordered.isApprox(d_reordered, 1e-6));

      // Now also check for each segment.
      for (size_t j = 0; j < segments.size(); ++j) {
        Eigen::VectorXd p_seg = segments[j][i].getCoefficients(0);
        REQUIRE(p_seg.isApprox(p.segment<N>(j * N), 1e-6));
      }
    }
  }
}
