...
TEST_CASE("Convolution", "[MavTrajectoryGeneration]") {
  ...
  REQUIRE(expected_convolution.isApprox(convolution.getCoefficients(), kEqualityResolution) == true);
}

TEST_CASE("FindMinMax", "[PolynomialTest]") {
  ...
  for (size_t i = 0; i < kNumPolynomials; i++) {
    ...
    REQUIRE(approxEqual(max_sampling.first, max_computing.first) == true);
    REQUIRE(approxEqual(min_sampling.first, min_computing.first) == true);
  }
