...
class RaycastRingBuffer {
 public:
  ...
  /// Decay the occupancy grid by calling addMiss to all occupied cells in the grid
  /// TODO(murt): Is there a faster way to achieve this without iterating over all voxels?
  void decay() {
    // Iterate over all voxels and update
    Vector3i min_idx(-_N,-_N,-_N);
    Vector3i max_idx(_N,_N,_N);
    for (int x = min_idx(0); x <= max_idx(0); ++x) {
      for (int y = min_idx(1); y <= max_idx(1); ++y) {
        for (int z = min_idx(2); z <= max_idx(2); ++z) {
          ...
          // Only decay the occupied data
          if (isOccupied(occupancy_data)) {
  ...
  void getCloudFree(PointCloud &cloud, Vector3 &offset_point) {
    occupancy_buffer_.getCloudHelper(cloud, offset_point,
                      [](const _Datatype & d) { return isFree(d);});
  }

  void getCloudOccupied(PointCloud &cloud, Vector3 &offset_point) {
      occupancy_buffer_.getCloudHelper(cloud, offset_point,
                      [](const _Datatype & d) { return isOccupied(d);});
  }

  void getCloudUpdated(PointCloud &cloud, Vector3 &offset_point) {
      flag_buffer_.getCloudHelper(cloud, offset_point,
 ...
 protected:
  ...
  void insertFree(const Vector3i &point_idx,
    ...
    for (float i = 0; i <= length; i += resolution_) {
