...
class RingBufferBase {
 public:
  ...
  //  Note: typedef std::vector <Vector4, Eigen::aligned_allocator<Vector4>> PointCloud;
  template<typename F>
  void getCloudHelper(std::vector <Vector4, Eigen::aligned_allocator<Vector4>> &cloud, Vector3 &offset_point, F func) {
    for (int x = 0; x < _N; x++) {
        for (int y = 0; y < _N; y++) {
            for (int z = 0; z < _N; z++) {
                getPoint(offset_, offset_point);
                ...
                if (func(data)) cloud.push_back(p);
            }
        }
    }
  }
