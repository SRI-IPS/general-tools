...
class EuclideanDistanceRingBuffer {
 public:
  ...
  inline bool insideVolume(const Vector3i &idx) {
    return distance_buffer_.insideVolume(idx);
  }
  ...
  void getCloudFree(PointCloud &cloud, Vector3 &offset_point) {
    occupancy_buffer_.getCloudFree(cloud, offset_point);
  }

  void getCloudOccupied(PointCloud &cloud, Vector3 &offset_point) {
    occupancy_buffer_.getCloudOccupied(cloud, offset_point);
  }

  void getCloudUpdated(PointCloud &cloud, Vector3 &offset_point) {
    occupancy_buffer_.getCloudUpdated(cloud, offset_point);
  }

  void getCloudDistance(PointCloud &cloud, _Scalar distance, Vector3 &offset_point) {
    distance_buffer_.getCloudHelper(cloud, offset_point,
                            [=](const _Scalar & d)
                            { return d <= distance;});
  }

  void decay() {
    occupancy_buffer_.decay();
  }
